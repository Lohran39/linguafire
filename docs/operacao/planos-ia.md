...
Todos os assinantes usam os limites da tabela, inclusive os antigos, conforme a decisão de 13/09/2026. A migração complementar `20260913-unify-ai-plan-quotas.sql` aplica a política 2 sem excluir assinaturas, alterar cobranças ou zerar o consumo registrado. Renovações e sincronizações Stripe ignoram metadata antiga de política e mantêm os novos limites. Os novos checkouts usam `subscription_data.metadata.ai_policy_version=2`. O preço cadastrado na Stripe é verificado antes do checkout (BRL, mensal, R$ 45/85). Nenhum preço remoto é alterado.
...
1. Aplicar `server/migrations/20260912-ai-plan-quotas.sql` e depois `server/migrations/20260913-unify-ai-plan-quotas.sql` no SQL Editor do Supabase antes de publicar o servidor. Se a primeira já foi aplicada, executar somente a segunda. Ambas são transacionais e reexecutáveis; respeitar essa ordem, pois a primeira contém a política histórica.
...
O teste PostgreSQL cria e destrói um cluster local isolado. Verifica concorrência real de 50 alunos, limite mensal e diário, rate limit por aluno, conversão da política antiga sem zerar consumo, virada de datas, devolução idempotente, agregação concorrente e permissões. O teste HTTP usa IA e armazenamento simulados: 50 alunos no mesmo IP passaram, com p95 local observado de 60 ms. Isso não comprova capacidade do Render, Redis, Supabase remoto ou Gemini; medir essas dependências no piloto com quotas e ambiente configurados. Não equivale a um teste com alunos reais.
