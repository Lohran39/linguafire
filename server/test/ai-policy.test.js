...
test('all subscribers use new quotas even with legacy Stripe metadata',()=>{
 ...
 const old=aiUsage({...user,ai_policy_version:1},now);assert.equal(old.limit,50);assert.equal(old.monthlyLimit,1000);assert.equal(old.legacy,false);
 ...
 assert.equal(subscriptionUpdates(sub,()=> 'max').ai_policy_version,2);
 assert.equal(subscriptionUpdates({...sub,metadata:{ai_policy_version:'1'}},()=> 'max').ai_daily_limit,150);
});
